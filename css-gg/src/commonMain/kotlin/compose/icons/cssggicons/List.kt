package compose.icons.cssggicons

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType.Companion.EvenOdd
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.StrokeCap.Companion.Butt
import androidx.compose.ui.graphics.StrokeJoin.Companion.Miter
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.ImageVector.Builder
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import compose.icons.CssGgIcons

public val CssGgIcons.List: ImageVector
    get() {
        if (_list != null) {
            return _list!!
        }
        _list = Builder(name = "List", defaultWidth = 24.0.dp, defaultHeight = 24.0.dp,
                viewportWidth = 24.0f, viewportHeight = 24.0f).apply {
            path(fill = SolidColor(Color(0xFF000000)), stroke = null, strokeLineWidth = 0.0f,
                    strokeLineCap = Butt, strokeLineJoin = Miter, strokeLineMiter = 4.0f,
                    pathFillType = EvenOdd) {
                moveTo(20.0f, 4.0f)
                horizontalLineTo(4.0f)
                curveTo(3.448f, 4.0f, 3.0f, 4.448f, 3.0f, 5.0f)
                verticalLineTo(19.0f)
                curveTo(3.0f, 19.552f, 3.448f, 20.0f, 4.0f, 20.0f)
                horizontalLineTo(20.0f)
                curveTo(20.552f, 20.0f, 21.0f, 19.552f, 21.0f, 19.0f)
                verticalLineTo(5.0f)
                curveTo(21.0f, 4.448f, 20.552f, 4.0f, 20.0f, 4.0f)
                close()
                moveTo(4.0f, 2.0f)
                curveTo(2.343f, 2.0f, 1.0f, 3.343f, 1.0f, 5.0f)
                verticalLineTo(19.0f)
                curveTo(1.0f, 20.657f, 2.343f, 22.0f, 4.0f, 22.0f)
                horizontalLineTo(20.0f)
                curveTo(21.657f, 22.0f, 23.0f, 20.657f, 23.0f, 19.0f)
                verticalLineTo(5.0f)
                curveTo(23.0f, 3.343f, 21.657f, 2.0f, 20.0f, 2.0f)
                horizontalLineTo(4.0f)
                close()
                moveTo(6.0f, 7.0f)
                horizontalLineTo(8.0f)
                verticalLineTo(9.0f)
                horizontalLineTo(6.0f)
                verticalLineTo(7.0f)
                close()
                moveTo(11.0f, 7.0f)
                curveTo(10.448f, 7.0f, 10.0f, 7.448f, 10.0f, 8.0f)
                curveTo(10.0f, 8.552f, 10.448f, 9.0f, 11.0f, 9.0f)
                horizontalLineTo(17.0f)
                curveTo(17.552f, 9.0f, 18.0f, 8.552f, 18.0f, 8.0f)
                curveTo(18.0f, 7.448f, 17.552f, 7.0f, 17.0f, 7.0f)
                horizontalLineTo(11.0f)
                close()
                moveTo(8.0f, 11.0f)
                horizontalLineTo(6.0f)
                verticalLineTo(13.0f)
                horizontalLineTo(8.0f)
                verticalLineTo(11.0f)
                close()
                moveTo(10.0f, 12.0f)
                curveTo(10.0f, 11.448f, 10.448f, 11.0f, 11.0f, 11.0f)
                horizontalLineTo(17.0f)
                curveTo(17.552f, 11.0f, 18.0f, 11.448f, 18.0f, 12.0f)
                curveTo(18.0f, 12.552f, 17.552f, 13.0f, 17.0f, 13.0f)
                horizontalLineTo(11.0f)
                curveTo(10.448f, 13.0f, 10.0f, 12.552f, 10.0f, 12.0f)
                close()
                moveTo(8.0f, 15.0f)
                horizontalLineTo(6.0f)
                verticalLineTo(17.0f)
                horizontalLineTo(8.0f)
                verticalLineTo(15.0f)
                close()
                moveTo(10.0f, 16.0f)
                curveTo(10.0f, 15.448f, 10.448f, 15.0f, 11.0f, 15.0f)
                horizontalLineTo(17.0f)
                curveTo(17.552f, 15.0f, 18.0f, 15.448f, 18.0f, 16.0f)
                curveTo(18.0f, 16.552f, 17.552f, 17.0f, 17.0f, 17.0f)
                horizontalLineTo(11.0f)
                curveTo(10.448f, 17.0f, 10.0f, 16.552f, 10.0f, 16.0f)
                close()
            }
        }
        .build()
        return _list!!
    }

private var _list: ImageVector? = null
