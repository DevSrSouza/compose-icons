package compose.icons.materialdesignicons

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType.Companion.NonZero
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.StrokeCap.Companion.Butt
import androidx.compose.ui.graphics.StrokeJoin.Companion.Miter
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.ImageVector.Builder
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import compose.icons.MaterialDesignIcons

public val MaterialDesignIcons.SpeakerMultiple: ImageVector
    get() {
        if (_speakerMultiple != null) {
            return _speakerMultiple!!
        }
        _speakerMultiple = Builder(name = "SpeakerMultiple", defaultWidth = 24.0.dp, defaultHeight =
                24.0.dp, viewportWidth = 24.0f, viewportHeight = 24.0f).apply {
            path(fill = SolidColor(Color(0xFF000000)), stroke = null, strokeLineWidth = 0.0f,
                    strokeLineCap = Butt, strokeLineJoin = Miter, strokeLineMiter = 4.0f,
                    pathFillType = NonZero) {
                moveTo(14.0f, 10.0f)
                arcTo(3.0f, 3.0f, 0.0f, false, false, 11.0f, 13.0f)
                arcTo(3.0f, 3.0f, 0.0f, false, false, 14.0f, 16.0f)
                arcTo(3.0f, 3.0f, 0.0f, false, false, 17.0f, 13.0f)
                arcTo(3.0f, 3.0f, 0.0f, false, false, 14.0f, 10.0f)
                moveTo(14.0f, 18.0f)
                arcTo(5.0f, 5.0f, 0.0f, false, true, 9.0f, 13.0f)
                arcTo(5.0f, 5.0f, 0.0f, false, true, 14.0f, 8.0f)
                arcTo(5.0f, 5.0f, 0.0f, false, true, 19.0f, 13.0f)
                arcTo(5.0f, 5.0f, 0.0f, false, true, 14.0f, 18.0f)
                moveTo(14.0f, 2.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, true, 16.0f, 4.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, true, 14.0f, 6.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, true, 12.0f, 4.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, true, 14.0f, 2.0f)
                moveTo(19.0f, 0.0f)
                horizontalLineTo(9.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 7.0f, 2.0f)
                verticalLineTo(18.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 9.0f, 20.0f)
                horizontalLineTo(19.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 21.0f, 18.0f)
                verticalLineTo(2.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 19.0f, 0.0f)
                moveTo(5.0f, 22.0f)
                horizontalLineTo(17.0f)
                verticalLineTo(24.0f)
                horizontalLineTo(5.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, true, 3.0f, 22.0f)
                verticalLineTo(4.0f)
                horizontalLineTo(5.0f)
            }
        }
        .build()
        return _speakerMultiple!!
    }

private var _speakerMultiple: ImageVector? = null
