package compose.icons.materialdesignicons

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType.Companion.NonZero
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.StrokeCap.Companion.Butt
import androidx.compose.ui.graphics.StrokeJoin.Companion.Miter
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.ImageVector.Builder
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import compose.icons.MaterialDesignIcons

public val MaterialDesignIcons.CashRefund: ImageVector
    get() {
        if (_cashRefund != null) {
            return _cashRefund!!
        }
        _cashRefund = Builder(name = "CashRefund", defaultWidth = 24.0.dp, defaultHeight = 24.0.dp,
                viewportWidth = 24.0f, viewportHeight = 24.0f).apply {
            path(fill = SolidColor(Color(0xFF000000)), stroke = null, strokeLineWidth = 0.0f,
                    strokeLineCap = Butt, strokeLineJoin = Miter, strokeLineMiter = 4.0f,
                    pathFillType = NonZero) {
                moveTo(12.3f, 8.93f)
                lineTo(9.88f, 6.5f)
                horizontalLineTo(15.5f)
                verticalLineTo(10.0f)
                horizontalLineTo(17.0f)
                verticalLineTo(5.0f)
                horizontalLineTo(9.88f)
                lineTo(12.3f, 2.57f)
                lineTo(11.24f, 1.5f)
                lineTo(7.0f, 5.75f)
                lineTo(11.24f, 10.0f)
                lineTo(12.3f, 8.93f)
                moveTo(12.0f, 14.0f)
                arcTo(3.0f, 3.0f, 0.0f, true, false, 15.0f, 17.0f)
                arcTo(3.0f, 3.0f, 0.0f, false, false, 12.0f, 14.0f)
                moveTo(3.0f, 11.0f)
                verticalLineTo(23.0f)
                horizontalLineTo(21.0f)
                verticalLineTo(11.0f)
                moveTo(19.0f, 19.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 17.0f, 21.0f)
                horizontalLineTo(7.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 5.0f, 19.0f)
                verticalLineTo(15.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 7.0f, 13.0f)
                horizontalLineTo(17.0f)
                arcTo(2.0f, 2.0f, 0.0f, false, false, 19.0f, 15.0f)
                close()
            }
        }
        .build()
        return _cashRefund!!
    }

private var _cashRefund: ImageVector? = null
