package compose.icons.materialdesignicons

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.PathFillType.Companion.NonZero
import androidx.compose.ui.graphics.SolidColor
import androidx.compose.ui.graphics.StrokeCap.Companion.Butt
import androidx.compose.ui.graphics.StrokeJoin.Companion.Miter
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.ImageVector.Builder
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp
import compose.icons.MaterialDesignIcons

public val MaterialDesignIcons.CeilingLightOutline: ImageVector
    get() {
        if (_ceilingLightOutline != null) {
            return _ceilingLightOutline!!
        }
        _ceilingLightOutline = Builder(name = "CeilingLightOutline", defaultWidth = 24.0.dp,
                defaultHeight = 24.0.dp, viewportWidth = 24.0f, viewportHeight = 24.0f).apply {
            path(fill = SolidColor(Color(0xFF000000)), stroke = null, strokeLineWidth = 0.0f,
                    strokeLineCap = Butt, strokeLineJoin = Miter, strokeLineMiter = 4.0f,
                    pathFillType = NonZero) {
                moveTo(14.76f, 11.0f)
                lineTo(16.76f, 15.0f)
                horizontalLineTo(7.24f)
                lineTo(9.24f, 11.0f)
                horizontalLineTo(14.76f)
                moveTo(13.0f, 4.0f)
                horizontalLineTo(11.0f)
                verticalLineTo(9.0f)
                horizontalLineTo(8.0f)
                lineTo(4.0f, 17.0f)
                horizontalLineTo(20.0f)
                lineTo(16.0f, 9.0f)
                horizontalLineTo(13.0f)
                verticalLineTo(4.0f)
                moveTo(14.0f, 18.0f)
                horizontalLineTo(10.0f)
                curveTo(10.0f, 19.11f, 10.9f, 20.0f, 12.0f, 20.0f)
                reflectiveCurveTo(14.0f, 19.11f, 14.0f, 18.0f)
                close()
            }
        }
        .build()
        return _ceilingLightOutline!!
    }

private var _ceilingLightOutline: ImageVector? = null
