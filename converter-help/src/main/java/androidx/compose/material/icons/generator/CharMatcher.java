...
public abstract class CharMatcher implements Predicate<Character> {
  ...
  // Constant matcher factory methods
    ...
    public  <T> T checkNotNull(T item){
  ...
  public static int checkPositionIndex(int index, int size) {
    ...
    if (index < 0 || index > size) {
      throw new IndexOutOfBoundsException("i don't know ");
    }
    return index;
  }
...
//      checkPositionIndex(start, length);
  ...
  static class RangesMatcher extends CharMatcher {
    ...
    public static void checkArgument(boolean x){
        if(!x) throw new IllegalArgumentException("your argument is wrong idiot");
    }
